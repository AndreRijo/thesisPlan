...
Partial replication is thus of interest for large-scale services.
...
%Namely, the partition of the data must be done in such a way so that for most (preferably all) transactions, the data required to satisfy the transaction is present in a single datacenter.

\todo{Maybe the paragraph above has too much text/detail?}
...
%Citations for wide-replication. I know COPS is one of them, probably Spanner classifies too
%Saturn is geo distributed
%References I need: internal sharding in datacenters; someone claiming costs are expensive with full replication; possibly some examples of high. Maybe someone claiming geo-distribution is hard due to latency vs strong consistency.
%Citation for user experience factor.
%Citation of wide-replication, as well as systems applying it
%Citations for strong consistent, wide/geo replicated systems.
%Citations for weak consistent, wide/geo replicated systems. Check if things like chainreplication as also done with geo in mind.
%Citations for systems trying to go around the "limitations" of each solution (e.g., going around limitations of strong or weak)
...
In this scenario however, having to coordinate with more than one datacenter may be acceptable, as the latency is still low enough (below 100ms  \footnote{For instance: Amazon's AWS servers in the United States have below 70ms latency among themselves, while the European's servers have below 50ms.}).
...
\todo{Some commentary about PotionDB of partial + geo replication.}

Another consideration when applying geo-replication is related with full replication.
By default, all data is replicated in every datacenter, possibly with sharding inside each datacenter.
However, as the number of datacenters increases, the replication process becomes more costly both in terms of storage, networking and processing power.
Partial replication can be used to minimize the referred issues, specially when data access presents some kind of locality.
