...
As discussed in Chapter \ref{cha:introduction}, existing systems face scalability, performance and cost issues due to ever growing user bases and data sizes. %TODO: I would like to mention specifically replication costs. Or no need?
...
%However, as noted in Section \ref{sec:data_access}, current solutions tend to have unacceptable drawbacks, such as 
However, as noted in Section~\ref{sec:data-access}, current solutions suffer from severe drawbacks, such as
...
We plan to address the aforementioned challenges alongside others discussed in the following sections.
...
In the next sections we detail each of our contributions and the main pillars of work needed to achieve our goals.
...
\begin{enumerate}
	...
	%\item Storage and update cost of views should be minimized as much as possible - as was seen in Section \ref{sec:data_access}, both have been a liability with adopting materialized views in commercial databases and is an hot topic of research.
...
The last two challenges are deeply related with the replication and transaction execution algorithms described in the next sections. \todo{This may sound like a contradiction, because then on the next two paragraphs, I say that the former has mostly implications on the transactional and the latter on the replication. But either requirement only works because of the combination of transactional and replication algorithms}
...
When preparing a transaction for replication, it must be ensured that all of the transaction's view updates are sent to all servers replicating said view(s), even if the base objects are not replicated in those servers. \todo{Is "even if the base objects are not" clear to what it is referring to?}
...
\begin{itemize}
	...
	\item Network efficient - in general, reduce the amount of data sent to each server. \todo{Network or data efficient? Thoughts?}
...
%11->
We also incorporate non-uniform replication, as it allows us to further reduce storage and network costs, specially for views.
...
The transactional algorithm is the one responsible for applying both updates and queries, while maintaining data consistency.
...
\begin{itemize}
	...
	\item Transactions received through replication must execute correctly - despite only receiving the operations that concern objects locally replicated (i.e., not the complete transaction), consistency must still be maintained; \todo{Is this clear? Please check on the .tex file the old version that was here. I don't know which one is better.}  
	%\item Transactions received though replication must execute correctly - despite only receiving the parts of the transaction for objects locally replicated, consistency must still be maintained;
...
We assume each object can be uniquely identified by a \emph{key}', and that the system is internally partitioned (sharded) in an arbitrary number of shards. \todo{Do I need to refer here to the partial replication subsection (i.e., where I talk about sharding vs partial replication)?}
...
Note that shards are independent of buckets - two objects belonging to different shards can have the same bucket. \todo{Is this last phrase needed, or is it already clear?}
...
%The existence of partitions lets read-only transactions to execute in parallel.
...
%Our algorithm uses a two-phase commit protocol (2PC, see Section \ref{subsec:existingSystems}) to coordinate transactions when updates are involved.
...
Our algorithm minimizes the required coordination by only coordinating shards involved in read-write transactions - other shards keep executing read-only transactions unaffected. \todo{Is this description enough? Or should I explain clearly that 2PC is, for each transaction, ran only in the shards of that transaction? I was afraid that would create confusion.}
...
This rebuilding process lets us reconstruct versions that may not even have been directly observed by the local replica. \todo{Do I need to explain this? It's hard to explain in without a long explanation. The idea is that we may want to rebuild a version that was not observed in the local replica: e.g., if replica A observes replica B at versions 10 and 20 (due to replication not happening in-between), we may still want to see version 15 of replica B at replica A. This is even more complicated as it may involve multiple replicas and thus different interleaving of operations. Hence the need to rebuild.}
...
\end{enumerate*}
Remarkably, step \ref{item:gc} is not straightforward, as it is challenging to identify if a version is safe to be garbage collected or not, as ongoing transactions may require access to old versions. %Could be shortened if needed; can try to avoid using the "or not".
...
We plan to address this by tracking ongoing transactions to determine the oldest version (\emph{V}) that is still relevant, and delete all versions older than \emph{V} during GC. \todo{Maybe here I should not say "we plan to improve on this by (...)" and instead say something like "a possible solution to improve on this is by (...)"?}
...
We believe further work with consistency in an interesting research avenue due to the implications views have in our system. \todo{Please check if the rest of this section makes this clear enough, or if I should add more references to how views complicate everything (basically the point is: must always ensure views + objects stay consistent no matter what we do.)}
...
This is useful to ensure, e.g., an item's stock is never negative, a sale matches an existing product, etc. \todo{Maybe this explanation is not needed, as I already explained it in Section \ref{subsec:invariants}?}
...
\end{itemize}
\todo{I don't think implementing the version management solution of Cure, a basic GC algorithm nor numeric invariants (bounded counter) count as contributions, correct? Nor does TPC-H's queries implementation nor benchmarking already done.}
...
Another noteworthy mention is the implementation in PotionDB of the complete TPC-H benchmark \cite{tpch}, including its dataset, updates and the full query set. %Should I mention it's something not done by many publications as it is very challenging? I only know of one publication that admits implementing only a subset of the queries.
...
Lastly, we also implemented a simple algorithm for executing transactions in a replica when some of the objects are not locally replicated. \todo{The sole reason for implementing some of the new CRDTs was to ensure queries with large result set or complex processing would still be efficient. Should I mention this or no need?}
\todo{Should the reference for executing transactions for non locally replicated objects be in the transactional algorithm section, or is it okay here. Also, any further details on it or ok like this?} 
