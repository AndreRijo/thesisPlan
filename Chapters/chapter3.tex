...
This work targets scalable data replication and management for geo-distributed services.
As discussed in Chapter~\ref{cha:introduction}, existing systems struggle with scalability, performance and replication costs as user bases and data volumes grow, and the situation is exacerbated by the increasing demand for analytical queries that must run in real time, over large data volumes, and without disrupting ongoing operations.
Chapter~\ref{cha:research_context} surveyed techniques relevant to this setting -- partial replication, non-uniform replication, causal consistency, materialized views, among others.
%which require analysing large amounts of data while the system is still running.
%However, as noted in Section \ref{sec:data-access}, current solutions tend to have unacceptable drawbacks, such as 
However, as noted in Section~\ref{sec:data-access}, current solutions tend to compromise on at least one of:
\begin{enumerate*}[label=(\roman*)]
\item data freshness and consistency;
\item query performance and its impact on the rest of the system;
\item infrastructure cost; and
\item synchronization overhead.
\end{enumerate*}

We propose to address these challenges with PotionDB, a geo-distributed database that combines and extends these techniques into a single, coherent design.
Our vision is to empower global-scale applications deployed across many data centers with: low replication costs achieved through data locality, partial and non-uniform replication; scalability across data centers via causal-based consistency; intra-data-center scalability via sharding; and near real-time analytical queries supported by materialized views that are automatically and incrementally maintained, even over partitioned data.
%as well as support partitioned data.
...
A central novelty of our work is the support of materialized views in a geo-distributed, weakly consistent, partially replicated setting.
This is particularly challenging because the views must reflect data that may not be entirely present in any single server, while remaining mutually consistent with the underlying objects.
The combination of partial replication, non-uniform replication and causal consistency, together with views integrated on top, is also novel.
As a result, complex recurring analytical queries can be answered with latency comparable to that of basic object reads.

All contributions are implemented in PotionDB, which serves both as a vehicle to showcase the proposed concepts and as a platform on which to evaluate their performance.
Figure~\ref{fig:arch} sketches PotionDB's high-level architecture; we revisit each component in the sections that follow.

\begin{figure}
	\centering
	\includegraphics[width=1.0\linewidth]{PotionDBArch2}
	\caption{PotionDB Architecture}
	\label{fig:arch}
\end{figure}

The remainder of this chapter details each pillar.
Section~\ref{sec:views_research_statement} presents the requirements and our solution for efficient analytical queries through materialized views.
Sections~\ref{sec:replication_algorithm} and~\ref{sec:transactional_algorithm} describe, respectively, our replication and transactional algorithms.
Section~\ref{sec:version_management} introduces our new solution for version management.
Section~\ref{sec:consistency_levels} discusses our plans to support different consistency guarantees, invariants, and a SQL-like interface.
Finally, Section~\ref{sec:contributions} summarizes both achieved and expected contributions.
...
A central goal of our work is to support recurring analytical queries in near real time, alongside the regular operational queries of an application.
Analytical queries may need to process millions of data objects to compute their answer, so a dedicated mechanism is required.
...
\begin{enumerate}
	...
	\item Storage and update cost of views must be minimized - as discussed in Section~\ref{sec:data-access}, both factors have hindered the adoption of materialized views in commercial databases and remain a hot topic of research;
	%\item Storage and update cost of views should be minimized as much as possible - as was seen in Section \ref{sec:data-access}, both have been a liability with adopting materialized views in commercial databases and is an hot topic of research.
%	\item Access to views must be efficient - queries to views should be in same order of complexity as queries to other objects.
	%Thus, the required data to reply to queries of views should be readily available without any complex or time-consuming calculations or communication.
	...
	%\item In each server, views must reflect all updates even for entries whose objects are not locally replicated, without breaking causality.
	%\item Views must be updated even for entries whose objects are not locally replicated, without breaking causality
\end{enumerate}
%TODO: Consider changing the itemize to an enumerate, for easier referencing on the explanation below.

%Mention they are objects like any other, but are updated automatically

We tackle these challenges as follows.
For efficiency, we rely on \emph{incremental view maintenance}: each update to an object generates one update per relevant view, modifying only the affected portion rather than recomputing the view from scratch.
In our PotionDB prototype, views are implemented as CRDTs, like other objects, so updating a view amounts to applying a CRDT operation.
Because views are CRDTs, queries are fast: the data required to answer them is already materialized in a form chosen for the query.
For example, a top-$k$ query is answered by a top-$k$ CRDT, while a query requiring per-entry sums is answered by a map CRDT with embedded counter CRDTs.

Storage and update costs are a major concern, as views are typically replicated in multiple (potentially all) data centers and may aggregate large amounts of data.
Minimizing both the storage footprint and the number of updates each data center applies to a view is therefore essential.
...
First, by selecting appropriate CRDTs we avoid storing unnecessary data.
...
%Secondly, by combining non-uniform replication with partial replication, we reduce both costs by, respectively, reducing the size and number of updates of each view and by replicating the view only where it is relevant.
%We detail this further in the next section.
%Secondly, by leveraging on non-uniform replication (Section \ref{subsec:nonuniform}), we can reduce storage and update costs further.
Secondly, by combining non-uniform and partial replication (Section \ref{sec:replication}), we reduce storage and update costs further.
 Non-uniform replication identifies which updates may affect the state observable by the query, replicating only such updates. %, effectively reducing the number of updates applied and replicated in each replica as well as the number of entries in the view.
...
%Secondly, by leveraging on non-uniform replication (see Section \ref{subsec:nonuniform}), we can reduce storage and update costs further.
...
%The \ref{item:objviewconsistency}th challenge is deeply related with the replication and transactional algorithms described in the next sections.
The last two challenges are intertwined with both the replication and transaction execution algorithms described in the next sections, but each has more pronounced implications on one of them.
We focus now on those implications, noting that meeting either requirement requires the cooperation of both algorithms.

Keeping views and their objects mutually consistent has implications mostly on the transactional algorithm.
First, transactions must be atomic: clients either observe all changes or none.
Second, whenever an object is updated, all related views must be updated within the same transaction.
Finally, the transactional and replication algorithms together must enable PSI+cset consistency (Section~\ref{sec:weak-consistency}) for our transactions, reducing the concurrency anomalies that would otherwise be observable under weak consistency.
%Additionally, despite our solution being a weakly consistent one, our transactional and replication algorithms should ensure 
%Finally, to reduce the amount of concurrency anomalies observable due to weak consistency, transactions must be applied according to their casual order.
...
The last challenge is mostly related to the replication algorithm.
When preparing a transaction for replication, all of the transaction's view updates must be delivered to every server replicating the affected view(s), even when those servers do not replicate the base objects from which the updates were derived.
This calls for a mechanism that identifies view updates and routes them to the appropriate view replicas alongside the remaining updates relevant to each of those replicas.

\section{Replication algorithms}
\label{sec:replication_algorithm}

Another crucial aspect of our work is the design of replication and transaction-execution algorithms suited to geo-replication.
Both must cope with, and whenever possible exploit, partial and non-uniform replication for efficiency, while also enforcing the view-related requirements identified in the previous section.
...
\begin{itemize}
	...
	%this implies avoiding blocking mechanisms or having to coordinate with faraway replicas to make progress;
	\item Network efficient - reduce the amount of data sent to each server, both in bytes transferred and in number of operations replicated.
...
%12->12 (OK as now the partitioning of a transaction is more clear and we state that the original order is kept.)
...
%Each object is assigned to one bucket, and each server can replicate one or more buckets.
...
%To replicate a transaction, we first organize the transaction's operations in by their buckets, maintaining for each bucket the original order of operations.
%To replicate a transaction, we partition its operations into groups based on their associated buckets, preserving the original order of operations within each group.
...
As an optimization, when several transactions are replicated together, the end-of-transaction marker only needs to be sent for the last one in the batch, reducing the messaging overhead.

%11->
We also incorporate non-uniform replication, as it allows us to further reduce storage and network costs, especially for views.
This is not straightforward, as in non-uniform replication,
%This has implications in our algorithm's design, as in non-uniform replication, 
...
%some operations that were previously not relevant (i.e., did not need to be replicated), may need to be replicated due to the execution of another operation.
...
%In terms of the replication algorithm, 
...
The transactional algorithm is responsible for executing both updates and queries while maintaining data consistency.
Specifically, it must ensure the following:
\begin{itemize}
	...
	\item Transactions received through replication must execute correctly - despite each server only receiving the subset of operations that concern locally replicated objects (not the complete transaction), consistency must still be maintained;	%\item Transactions received though replication must execute correctly - despite only receiving the parts of the transaction for objects locally replicated, consistency must still be maintained;
...
%6->8 (OK, added a lot more information.)

We assume each object can be uniquely identified by a \emph{key}, and that the system is internally partitioned (sharded) in an arbitrary number of shards (see Section~\ref{subsec:partial} for the distinction between sharding and partial replication).
Each shard stores a subset of the objects' keyspace.
%This allows us to explore parallelism by assigning different shards to different servers or CPU cores - in our work, we assume the later.
This allows us to exploit parallelism by mapping shards either to different servers or to different CPU cores; we assume the latter in this work.
Our transactional algorithm (together with the replication algorithm) ensures PSI+cset (see Section \ref{sec:weak-consistency}) consistency, which can informally be stated as SI at the level of a single server, and causal consistency across servers.
Note that shards are independent of buckets - two objects belonging to different shards can be assigned to the same bucket.
%We assume the system is internally partitioned (sharded) in an arbitrary number of ``shards", with each shard storing a subset of the keyspace of objects.
...
%7->6
Sharding allows read-only transactions -- and even individual queries within the same transaction -- to execute in parallel, considerably speeding up transaction execution.
Each transaction is associated with a version identifier (e.g., a vector clock).
When executing queries, each shard uses this identifier to read the correct version of each object (version management is detailed in the next section), so that queries always observe a consistent snapshot of the database.

%8->7
%On the other hand, 
Conversely, read-write transactions may have to coordinate with other transactions to prevent conflicts.
%Our algorithm uses a two-phase commit protocol (2PC, see Section \ref{sec:existing-systems}) to coordinate transactions when updates are involved.
Our algorithm uses a two-phase commit protocol (2PC, see Section \ref{sec:existing-systems}) for coordinating transactions involving updates.
Since shards are on the same machine, 2PC executes very quickly, as communication is only between threads.
Our algorithm minimizes the required coordination by running 2PC, for each transaction, only over the shards involved in that transaction; the remaining shards keep executing read-only transactions unaffected.
%Our algorithm ensures the coordination is only done between shards involved in read-write transactions - other partitions may keep executing read-only transactions unaffected.
%In fact, if two read-write transactions operate in distinct subsets of shards, they will execute in parallel.
In fact, two read-write transactions operating in distinct subsets of shards will execute in parallel.

%Read-write transactions may require coordination to prevent conflicts.
%Our algorithm uses a two-phase commit protocol (2PC, see Section \ref{sec:existing-systems}) for coordinating transactions involving updates.
%Since shards are co-located on the same machine, 2PC runs quickly via inter-thread communication.
%Coordination is limited to shards involved in a given read-write transaction—others continue processing read-only transactions independently.
%As a result, two read-write transactions touching disjoint shard sets execute in parallel.
...
%ensuring both the view's and the objects' updates become visible at the same time.

%Using 2PC and object versions ensures all transaction updates become visible at a single point in time.
%On commit, updated objects move to a new version marked by the transaction’s commit clock.
%This version is only made visible after all shards complete the commit, ensuring simultaneous visibility across shards.
%Consistency between views and their objects is maintained by grouping their updates in the same transaction, so both become visible together.

%16->14. Would be nice to compact further.
...
%A simple transaction rebuild algorithm can consist in grouping all buckets received, with all updates in a bucket according to the original order, and the order between buckets can be any.
...
%To support transactions involving non-uniformly replicated objects, we proceed as follows:
%If such a transaction generates new operations, they are grouped into a new transaction using the involved shards.
%Once this new transaction commits on the relevant shards, it is passed to the replication algorithm.

A current limitation, common to both the replication and transactional algorithms, is that buckets are static: the set of buckets a server replicates does not change.
As discussed in Section~\ref{subsec:partial}, for some applications access patterns or data locality itself may shift over time (e.g., a user relocating to another continent).
...
%The last goal is particularly challenging, as repartitioning may happen while multiple transactions are ongoing, potentially for the buckets being added/removed.
...
PotionDB already supports new nodes joining the system and basic data transfer; further protocol work is required to achieve the remaining goals.
...
%Our transactional algorithm ensures consistency by providing, for each transaction, a consistent snapshot of the database, according to the transaction's version identifier.
%Thus, some mechanism to provide old versions of objects according to a given version identifier is needed.
...
%10->10 (added phrase regarding memory footprint)
Some solutions, e.g., Cure~\cite{cure}, keep multiple old versions in memory and garbage-collect them when no longer needed.
This can lead to an unreasonable memory footprint for large objects such as views.
We therefore explore a different approach: keep only the latest version (optionally caching a few extra versions for performance, but not as a requirement) and, alongside it, two logs per object -- the list of \emph{operations} executed and the list of their \emph{effects}.
We define an operation's \emph{effect} as the changes that operation produced on the object when it was executed.
Both logs are needed because the same operation may produce different changes depending on when it was executed.
For example, removing an element from a set may or may not actually remove it, depending on whether concurrent adds for the same element exist.

%8->8. Added the line to simplicity and rewrote the explanation of version rebuilding.
For simplicity (but not as a requirement), we assume vector clocks are used as version identifiers.
Given a target version and the current version, the algorithm proceeds in two steps.
First, we ``undo'' effects until reaching the highest version whose vector clock is component-wise less than or equal to the target.
Then we re-apply the operations needed to reach the target vector clock, in original order whenever they are causally related; concurrent operations may be re-applied in any order.
This rebuilding process lets us reconstruct versions that may not have been directly observed by the local replica.
For example, if replica $A$ observed replica $B$ at versions 10 and 20 but never at any version in between, a transaction at $A$ may still target version 15 of $B$; with multiple replicas, this can involve different interleavings of concurrent operations, hence the need for rebuilding.
%The re-application of operations is needed as the requested version may be a version that was not observed directly by the replica, due to a 
%The re-application of operations is needed due to concurrent operations.
%Concurrent operations may be re-applied in any order, but causally related operations must be applied in order.
%E.g., operation A may be concurrent to operation B and C, but B and C are causally related - A can be applied at any point, but B and C must be applied in order.
...
We identify the following next steps on this topic:
\begin{enumerate*}[label=(\roman*)]
	...
	%\item \label{item:benchmarkVms}implement other version management solutions and benchmark them, to assess our solution comparatively in terms of performance and space used in different scenarios.	
\end{enumerate*}
Step~\ref{item:gc} is not straightforward: identifying which versions are safe to delete is hard because ongoing transactions may still require access to old versions, and determining the latest common version identifier across shards is also necessary, since some shards may lag behind.

Step~\ref{item:vms} is already completed.
Step~\ref{item:gc} is ongoing: we have designed and implemented a basic GC algorithm that determines a safe clock across shards, but takes a cautious approach to ongoing transactions by delaying data deletion by a few seconds; long-running transactions may still abort.
A possible solution is to track ongoing transactions to determine the oldest version $V$ that is still relevant, and delete all versions older than $V$ during GC.
For step~\ref{item:benchmarkVms}, we have already implemented a version manager similar to Cure's~\cite{cure}; further alternatives and the comparative benchmarking remain as future work.


%We plan to benchmark our version management solution, comparing it with other existing solutions and assess its advantages and disadvantages in different scenarios, both in terms of performance and space used.
%To ensure our solution remains performant as more updates are applied to each object, a garbage collection algorithm for version management is essential, as otherwise version metadata will grow indefinitely.
%Effective support for garbage collection is not straightforward, as we must carefully determine which version(s) are safe to be garbage collected, as ongoing transactions may need to access old versions.
%If a too recent version is deleted, ongoing transactions may have to be aborted and thus negatively affect performance and user experience.

%We have implemented the version management described above and, more recently, also a solution similar to Cure \cite{cure}, i.e., to keep all observed versions.
%On the next steps of our work, we plan to thoroughly compare the performance of both solutions, accessing the advantages and disadvantages of our solution in multiple scenarios in terms of performance and space used.
%Potentially we may also implement more alternative version management solutions.
%Essential to 
...
%
%\todo{The text above may need to be updated. We already support the other version, we have GC but both have not been evaluated for performance.}

%\section{Consistency levels}
\section{Consistency levels and Data Access}
\label{sec:consistency_levels}

%Keep initial paragraph of motivation + our current solution. (albeit adapt/rewrite)
%Discuss multiple consistency levels (but shorter) and the challenging requirement of still needing to keep views + objects consistent to support the stronger consistency levels
%Invariants (update to say we already have bounded counter; views+objects have implicit referential integrity by forcing view updates to be bundled and applied with objects' updates.)
%Mix in the above with the desirability of SQL for PotionDB, as it would allow to easily specify expressive queries that rely on PotionDB's views. Mention the inherent consistency challenges and invariants to support. Mention the extra challenge of supporting invariants on views.

Our algorithms are designed to provide PSI+cset consistency.
The prototype currently supports both PSI+cset and read-committed consistency, although only one can be selected at a time for all transactions.
Read-committed provides weaker guarantees but enables higher throughput.
Consistency is a particularly interesting research avenue in our setting because of the implications of views: regardless of the chosen consistency level, views and their referred objects must remain mutually consistent, which complicates the design space of any new consistency mechanism we may introduce.
...
%10->10
One possible research direction is to support multiple consistency levels and let each transaction choose the level it needs, in the spirit of red-blue consistency~\cite{li2012making}.
Some queries (e.g., statistics read from a view) tolerate weaker consistency in exchange for better performance, while transactions with strong-consistency requirements (e.g., banking) can request stronger guarantees.
A weakly consistent transaction should not break the database's consistency invariants -- in particular, view-object consistency -- so that strongly consistent transactions can still execute correctly.
One way to enforce this is to require update transactions to remain atomic, while letting read-only transactions follow their requested consistency level.
For example, read-committed reads can return any already-committed version directly, skipping internal coordination for improved performance.

%One possibility would be to support multiple consistency levels for transactions.
%This would imply supporting transactions executing with different consistency levels, as done e.g. with red-blue consistency. %TODO: Citation.
...
%Other queries requiring stronger consistency can request so, executing a bit slower but with stronger guarantees as a result.
%One way to ensure this is to make the reads responsible for ensuring the desired consistency level.
%I.e., transactions with writes would still be atomic to ensure objects (and their views) stay consistent, while reads executing under lower consistency guarantees can, e.g., read objects at different versions without internal coordination to improve performance. %NOTE: This is what we already do actually... maybe make it more clear/precise?

%11->14
A complementary approach is to strengthen consistency guarantees through invariants -- user-defined conditions over objects that must hold at all times.
As discussed in Section~\ref{sec:data-safety}, invariants are useful to enforce constraints such as ``an item's stock never becomes negative'' or ``a sale always refers to an existing product''.
Invariants are challenging under weak consistency, as transactions across servers execute without coordination.
We already support numerical invariants through Bounded Counters~\cite{boundedCounter}, and referential integrity between views and their objects by coupling their updates within the same transaction.
However, referential integrity across arbitrary objects remains a challenge: violating updates can occur in concurrent transactions on different servers.
We plan to address this alongside novel solutions for other classes of invariants, such as uniqueness and disjunction, since the existing solutions discussed in Section~\ref{sec:data-safety} have significant shortcomings.
Furthermore, we plan to support invariants over materialized views, which is particularly challenging because such invariants impose constraints on the state of related objects.
To the best of our knowledge, invariant support over materialized views under weak consistency is novel.
...
%Supporting invariants in materialized views would be extra challenging, as these invariants would have implications on the state of many other objects, which would need invariants too. %Point back to the Invariant section and shorten the description here. Also, state what we already implemented, and possible future work.
...
%The possibilities above are, indeed, some possibilities we may take, but it is not yet set on stone and is thus subject to changes. %TODO: Definitely need to rewrite this.
%
...
This section summarizes both the contributions already achieved and those expected from the remaining work.
The contributions already in place are:
...
\begin{itemize}
	...
	\item Support for materialized views under weak consistency, with updates to objects and views visible atomically;
	\item Support for views that refer to data not replicated locally, together with non-uniform and partial replication of the views themselves to reduce their storage and replication costs;
	\item A new technique for incremental view maintenance based on CRDT update operations;
	\item An algorithm that reconstructs the state of any object at any given point in time from the current state plus the operations and effects logs;
	\item An implementation in the PotionDB prototype of all the features above.
\end{itemize}

We highlight the novelty of our replication algorithm, which supports partial and non-uniform replication efficiently in the same design.
Likewise, supporting materialized views in a geo-distributed, weakly consistent, partially and non-uniformly replicated setting -- without requiring all relevant objects to reside in a single server -- is novel, as is the incremental maintenance of views via CRDT update operations.

Although not contributions in themselves, two engineering efforts are worth noting.
First, considerable work has gone into supporting a wide range of CRDTs in PotionDB, both to ease application development and to enable expressive queries; this required extensions and adaptations of existing CRDT designs as well as new CRDTs, with several specifically designed to keep queries with large result sets or complex processing efficient.
Second, we implemented the complete TPC-H benchmark~\cite{tpch} (dataset, updates and full query set) in PotionDB; since TPC-H targets relational SQL databases, this required mapping tables onto PotionDB's objects and defining appropriate views per query.
Beyond evaluation, this exercise highlights PotionDB's expressiveness, since its views support all TPC-H queries.
%The main goals of implementing TPC-H's benchmark is to both evaluate PotionDB's performance and demonstrate its interface's expressiveness.
We also implemented a simple algorithm for executing transactions in a replica when some of the objects accessed by the transaction are not locally replicated; the replica acts as a coordinator that fetches the missing data from a remote replica before completing the transaction locally.
...
\begin{itemize}
	...
	%\item A garbage collection algorithm for systems implementing PSI+cset with vector clocks. This algorithm will be able to determine a safe clock in all partitions and do the cleaning without affecting considerably the system's performance;
...
%3->?
Importantly, every contribution above must preserve the consistency between views and their referred objects, which significantly amplifies both the complexity and the novelty of the expected results.
%It is noteworthy that the aforementioned contributions always have to take in consideration the consistency between views and their objects, which makes these expected contributions more challenging and novel. %TODO: Update this to make it more "impactful".
...
%	\item Storage and update cost of views should be minimized when possible - as was seen in Section \ref{sec:data-access}, both have been a problem with adopting materialized views in commercial databases and is an hot topic of research.
%	\end{itemize}
...
%Causal consistency also prevents some concurrency anomalies from being observed, as discussed in Section \ref{sec:weak-consistency}.
...
%\end{itemize}
%
%Internally, our Transaction Manager uses a local two-phase commit protocol (2PC, check Section \ref{sec:existing-systems}) to coordinate the partitions on a read-write transaction.
...
%Some of the systems analysed in Section \ref{sec:existing-systems} include support for dynamic partitioning.
%
